    ...
    constructor() {
        ...
        this.properties = [
            ...
            },
            {
                ...
                name: 'authUrl',
                type: 'hidden',
                default: '={{((($credentials.auth0CustomDomain || $credentials.auth0Domain || "").trim().replace(/^https?:\\/\\//, "").replace(/\\/.*$/, "")).match(/^[A-Za-z0-9-]+(?:\\.[A-Za-z0-9-]+)+$/)) ? "https://" + (($credentials.auth0CustomDomain || $credentials.auth0Domain || "").trim().replace(/^https?:\\/\\//, "").replace(/\\/.*$/, "")) + "/authorize" : ""}}',
                required: true,
            },
            {
                ...
                name: 'accessTokenUrl',
                type: 'hidden',
                default: '={{((($credentials.auth0CustomDomain || $credentials.auth0Domain || "").trim().replace(/^https?:\\/\\//, "").replace(/\\/.*$/, "")).match(/^[A-Za-z0-9-]+(?:\\.[A-Za-z0-9-]+)+$/)) ? "https://" + (($credentials.auth0CustomDomain || $credentials.auth0Domain || "").trim().replace(/^https?:\\/\\//, "").replace(/\\/.*$/, "")) + "/oauth/token" : ""}}',
                required: true,
            },
            {
